...
if __name__ == "__main__":
    ...
    with st.form(key="onboarding"):
        ...
        morningTaskTime = st.time_input("Morning Task Time", value=time(8,30,0) ,step=60*5)
        timeFrame = st.slider("Time Frame (months)", min_value=1, max_value=12, value=6, step=1)
        questionnaireTime = st.time_input("Questionnaire Time", value=time(18,30,0) ,step=60*5)
        ...
        if submit_button:
            ...
            if email == "" or password == "":
                ...
                err = onBoardParticipant(email, password, usrInfo, questionnaireInfo, structure)
                if err != "":
                    st.error(err)  
                else:
